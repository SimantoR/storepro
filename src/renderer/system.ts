...
export function init() {
  const rootTask = new Promise<void>(
  ...
  const logTask = new Promise<void>(
    (resolve: () => void, reject: (err: Error) => void) => {
  ...
  const cacheTask = new Promise<void>(
    (resolve: () => void, reject: (err: Error) => void) => {
      if (!fs.existsSync(CACHE_DIR)) {
        fs.mkdir(CACHE_DIR).catch((err: Error) => {
          if (err) reject(err);
          else resolve();
        });
      }
...
export function logErr(err: Error): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    const outPath = path.resolve(
      LOGDIR_PATH,
      new Date().toString('dd-MM-yyyy') + '.json'
    );
    const output = `${new Date().toString('hh:mm:ss')}, ${
      err?.message
...
export function loadMenu(): Promise<MenuButtonProps[] | undefined> {
  return new Promise<MenuButtonProps[] | undefined>((resolve, reject) => {
    ...
    screenshot({ format: 'png' })
      .then(function (buffer: any) {
        jimp
          .read(buffer)
          .then((image) => {
            image.quality(80).write(outPath);
            resolve();
          })
          .catch((err) => reject(err));
      })
